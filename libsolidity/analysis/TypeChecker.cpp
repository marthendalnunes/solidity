		...
		if (auto var = dynamic_cast<VariableDeclaration const*>(declaration))
		{
			...
			if (!identifierInfo.suffix.empty())
			{
				...
				if (var->isStateVariable() || var->type()->dataStoredIn(DataLocation::Storage))
				{
					...
					// bytes calldata or string calldata
					*var->type() == ArrayType(DataLocation::CallData, true) ||
					*var->type() == ArrayType(DataLocation::CallData, false)
				)
				{
			...
			else if (var->type()->sizeOnStack() != 1)
			{
				...
				else if (var->type()->dataStoredIn(DataLocation::CallData))
